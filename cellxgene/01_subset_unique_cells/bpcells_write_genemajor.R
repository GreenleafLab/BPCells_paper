...
# Use less memory for sorting on the laptop, since 16 threads can use up a lot
if (threads == 16L) {
    sort_bytes <- 2^29
} else {
    sort_bytes <- 2^30
...
write_chunk <- function(chunk_num) {
    ...
    mat[, obs_idx[rows]] |>
        transpose_storage_order(sprintf("%s/%03d", output_dir, chunk_num), sort_bytes=sort_bytes)
}

timing <- system.time({
    res <- parallel::mclapply(mc.cores=threads, mc.preschedule=FALSE,
