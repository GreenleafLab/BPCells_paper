...
stopifnot(input_type %in% c(
    ...
    "bpcells_cellmajor_lognorm",
    "bpcells_genemajor_lognorm"
))
...
if (str_detect(input_type, "bpcells")) {
    ...
    if (str_detect(input_type, "_(log)?norm")) {
        ...
        if (str_detect(input_type, "_lognorm")) {
            mat@matrix_list <- lapply(mat@matrix_list, function(m) log1p(m*10000))
        }
    } 
